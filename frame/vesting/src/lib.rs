...
mod tests {
	...
	type EtpError = hyperspace_balances::Error<Test, EtpInstance>;
	...
	fn check_vesting_status() {
		ExtBuilder::default()
			...
			.build()
			.execute_with(|| {
				let user1_free_balance = Etp::free_balance(&1);
				let user2_free_balance = Etp::free_balance(&2);
				let user12_free_balance = Etp::free_balance(&12);
	...
	fn vested_balance_should_transfer() {
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_ok!(Etp::transfer(Some(1).into(), 2, 55));
			});
	}
	...
	fn extra_balance_should_transfer() {
		ExtBuilder::default()
			...
			.build()
			.execute_with(|| {
				assert_ok!(Etp::transfer(Some(3).into(), 1, 100));
				assert_ok!(Etp::transfer(Some(3).into(), 2, 100));
				...
				assert_eq!(Vesting::vesting_balance(&1), Some(45));
				assert_ok!(Vesting::vest(Some(1).into()));
				assert_ok!(Etp::transfer(Some(1).into(), 3, 155)); // Account 1 can send extra units gained

				// Account 2 has no units vested at block 1, but gained 100
				assert_eq!(Vesting::vesting_balance(&2), Some(200));
				assert_ok!(Vesting::vest(Some(2).into()));
				assert_ok!(Etp::transfer(Some(2).into(), 3, 100)); // Account 2 can send extra units gained
			});
	}
	...
	fn liquid_funds_should_transfer_with_delayed_vesting() {
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_ok!(Etp::transfer(Some(12).into(), 3, 256 * 5));
			});
	}
	...
	fn vested_transfer_works() {
		ExtBuilder::default()
			...
			.build()
			.execute_with(|| {
				let user3_free_balance = Etp::free_balance(&3);
				let user4_free_balance = Etp::free_balance(&4);
				...
				let user3_free_balance_updated = Etp::free_balance(&3);
				assert_eq!(user3_free_balance_updated, 256 * 25);
				let user4_free_balance_updated = Etp::free_balance(&4);
