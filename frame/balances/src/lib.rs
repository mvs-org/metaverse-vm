...
//! - [`LockableCurrency`](hyperspace_support::traits::LockableCurrency): Functions for
...
pub mod pallet {
	...
	use hyperspace_support::{balance::*, impl_rpc, traits::BalanceInfo};
...
pub mod migration {
	...
	pub mod try_runtime {
		...
		pub fn pre_migrate() -> Result<(), &'static str> {
			...
			log::info!("Migrating `Instance0Hyperspace` to `Balances`...");
			migration::migrate(b"Instance0HyperspaceBalances", b"Balances");
	...
	pub fn migrate(old_pallet_name: &[u8], new_pallet_name: &[u8]) {
		// Balances: Instance0HyperspaceBalances
		// Dna: Instance1HyperspaceBalances
