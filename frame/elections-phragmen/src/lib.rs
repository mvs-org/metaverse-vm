...
decl_storage! {
	trait Store for Module<T: Trait> as HyperspacePhragmenElection {
		// ---- State
	...
	fn do_phragmen() {
		...
		.map(
			|ElectionResult {
				...
				let mut prime_votes: Vec<_> = new_members_sorted_by_id
					.iter()
				...
				// they are not members.
				{
					...
					debug_assert!(outgoing
						.iter()
						.all(|o| !new_runners_up_ids_sorted.contains(o)));
					to_burn_bond.extend(
						outgoing
							.iter()
...
mod tests {
	...
	frame_support::construct_runtime!(
			...
			UncheckedExtrinsic = UncheckedExtrinsic
		{
			...
			Balances: hyperspace_balances::<Instance0>::{Module, Call, Config<T>, Event<T>},
	...
	impl ExtBuilder {
		...
		pub fn build_and_execute(self, test: impl FnOnce() -> ()) {
			...
			let mut ext: sp_io::TestExternalities = GenesisConfig {
				hyperspace_balances_Instance0: Some(hyperspace_balances::GenesisConfig::<
					Test,
					EtpInstance,
				> {
					balances: vec![
