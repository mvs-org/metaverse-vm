...
use crate::{
	account_basic::{RemainBalanceOp, EtpRemainBalance},
	Call, *,
};
use codec::Decode;
...
macro_rules! assert_balance {
	($evm_address:expr, $balance:expr, $left:expr, $right:expr) => {
		let account_id =
			<Test as hyperspace_evm::Config>::AddressMapping::into_account_id($evm_address);
		assert_eq!(
			<Test as hyperspace_evm::Config>::EtpAccountBasic::account_basic(&$evm_address).balance,
			$balance
		);
		assert_eq!(
		...
		assert_eq!(
			<EtpRemainBalance as RemainBalanceOp<Test, u64>>::remaining_balance(&account_id),
			$right
		);
	};
}
...
fn transaction_should_increment_nonce() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			<Test as hyperspace_evm::Config>::EtpAccountBasic::account_basic(&alice.address).nonce,
			U256::from(1)
		);
	});
}
...
fn withdraw_with_enough_balance() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			<Test as hyperspace_evm::Config>::EtpAccountBasic::account_basic(&alice.address).balance,
		...
		assert_eq!(
			<Test as Config>::EtpCurrency::free_balance(dest),
			30000000000
		);
	});
}
...
fn mutate_account_works_well() {
	...
	ext.execute_with(|| {
		...
		<Test as hyperspace_evm::Config>::EtpAccountBasic::mutate_account_basic(
			&test_addr,
			Account {
				nonce: U256::from(0),
