...
// --- hyperspace ---
use crate::{self as hyperspace_democracy, *};
use hyperspace_balances::Error as BalancesError;
...
impl Filter<Call> for BaseFilter {
	fn filter(call: &Call) -> bool {
		!matches!(
			call,
			&Call::Balances(hyperspace_balances::Call::set_balance(..))
		)
	}
}
...
pub fn new_test_ext() -> sp_io::TestExternalities {
	...
	.unwrap();
	hyperspace_democracy::GenesisConfig::default()
		.assimilate_storage(&mut t)
		.unwrap();
...
fn set_balance_proposal(value: u64) -> Vec<u8> {
	Call::Balances(hyperspace_balances::Call::set_balance(42, value, 0)).encode()
}
