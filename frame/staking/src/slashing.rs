...
pub(crate) type RKT<T> = RK<EtpBalance<T>, DnaBalance<T>>;
...
pub(crate) struct SpanRecord<EtpBalance, DnaBalance> {
	slashed: RK<EtpBalance, DnaBalance>,
	paid_out: RK<EtpBalance, DnaBalance>,
}

impl<EtpBalance, DnaBalance> SpanRecord<EtpBalance, DnaBalance> {
	/// The value of stash balance slashed in this span.
	#[cfg(test)]
	pub(crate) fn amount_slashed(&self) -> &RK<EtpBalance, DnaBalance> {
		&self.slashed
	}
}
...
pub(crate) struct SlashParams<'a, T: 'a + Config> {
	...
	pub(crate) exposure: &'a Exposure<T::AccountId, EtpBalance<T>, DnaBalance<T>>,
...
pub(crate) fn compute_slash<T: Config>(
...
) -> Option<UnappliedSlash<T::AccountId, EtpBalance<T>, DnaBalance<T>>> {
	let SlashParams {
		stash,
		slash,
		exposure,
	...
	let own_slash = RK {
		r: slash * exposure.own_etp_balance,
	...
	for nominator in &exposure.others {
		...
		let era_slash = {
			let own_slash_prior = RK {
				r: prior_slash_p * nominator.etp_balance,
				k: prior_slash_p * nominator.dna_balance,
			};
			let own_slash_by_validator = RK {
				r: slash * nominator.etp_balance,
...
pub fn do_slash<T: Config>(
	...
	slashed_etp: &mut EtpNegativeImbalance<T>,
	...
	if !slash_etp.is_zero() {
		slashed = true;

		let (imbalance, missing) = T::EtpCurrency::slash(stash, slash_etp);

		slashed_etp.subsume(imbalance);
...
pub(crate) fn apply_slash<T: Config>(
	unapplied_slash: UnappliedSlash<T::AccountId, EtpBalance<T>, DnaBalance<T>>,
) {
	let mut slashed_etp = <EtpNegativeImbalance<T>>::zero();
...
fn pay_reporters<T: Config>(
	...
	let etp_reward_payout = reward_payout.r.min(slashed_etp.peek());
	let (mut etp_reward_payout, mut etp_slashed) = slashed_etp.split(etp_reward_payout);
	...
	let etp_per_reporter = etp_reward_payout.peek() / (reporters.len() as u32).into();
	let dna_per_reporter = dna_reward_payout.peek() / (reporters.len() as u32).into();

	for reporter in reporters {
		if !etp_per_reporter.is_zero() {
			let (etp_reporter_reward, etp_rest) = etp_reward_payout.split(etp_per_reporter);
			etp_reward_payout = etp_rest;

			// this cancels out the reporter reward imbalance internally, leading
			// to no change in total issuance.
			T::EtpCurrency::resolve_creating(reporter, etp_reporter_reward);
		}
	...
	etp_slashed.subsume(etp_reward_payout); // remainder of reward division remains.
	T::EtpSlash::on_unbalanced(etp_slashed);
