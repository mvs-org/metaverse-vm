...
impl SubstrateCli for Cli {
	...
	fn support_url() -> String {
		"https://github.com/betelgeuse/betelgeuse/issues".into()
	}
	...
	fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
		Ok(match id {
			"" | "betelgeuse" | "mainnet" => Box::new(chain_spec::mainnet_config()),
...
pub fn run() -> sc_cli::Result<()> {
	...
	let mut randomx_config = betelgeuse_pow::compute::Config::new();
	...
	let _ = betelgeuse_pow::compute::set_global_config(randomx_config);
	...
	match &cli.subcommand {
		...
		Some(Subcommand::ExportBuiltinWasm(cmd)) => {
			let wasm_binary_bloaty = betelgeuse_runtime::WASM_BINARY_BLOATY.ok_or("Wasm binary not available".to_string())?;
			let wasm_binary = betelgeuse_runtime::WASM_BINARY.ok_or("Compact Wasm binary not available".to_string())?;
			...
			let folder = PathBuf::from(cmd.folder.clone());
			{
				...
				path.push("betelgeuse_runtime.compact.wasm");
			...
			{
				let mut path = folder.clone();
				path.push("betelgeuse_runtime.wasm");
				let mut file = File::create(path)?;
		...
		Some(Subcommand::ImportMiningKey(cmd)) => {
			...
			runner.sync_run(|config| {
				...
				let pair = betelgeuse_pow::app::Pair::from_string(
					&cmd.suri,
					None,
		...
		Some(Subcommand::GenerateMiningKey(cmd)) => {
			...
			runner.sync_run(|config| {
				...
				let (pair, phrase, _) = betelgeuse_pow::app::Pair::generate_with_phrase(None);
		...
		Some(Subcommand::Benchmark(cmd)) => {
			if cfg!(feature = "runtime-benchmarks") {
				...
				runner.sync_run(|config| cmd.run::<betelgeuse_runtime::Block, service::Executor>(config))
			} else {
